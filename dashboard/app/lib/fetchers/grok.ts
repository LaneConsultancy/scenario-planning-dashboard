...
const INDICATOR_PROMPTS: IndicatorPrompt[] = [
  {
    id: "iea-disruption",
...
export const EXPECTED_GROK_IDS: readonly string[] = INDICATOR_PROMPTS.map((p) => p.id);
const VALID_GROK_IDS = new Set(EXPECTED_GROK_IDS);
...
Indicators to assess:`;
  ...
  const subset = onlyIds
    ? INDICATOR_PROMPTS.filter((p) => onlyIds.has(p.id))
    : INDICATOR_PROMPTS;

  const indicatorBlocks = subset
    .map((indicator, index) => {
...
async function callGrokOnce(client: GrokClient, prompt: string): Promise<GrokAssessment[]> {
...
function missingIds(expected: readonly string[], got: GrokAssessment[]): string[] {
  const have = new Set(got.map((a) => a.id));
...
export async function fetchGrokAssessments(
  ...
  for (const assessment of retryPass) {
